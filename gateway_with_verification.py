    ...
    def test_url(base_url):
        ...
        for vuln_type, payload in test_payloads.items():
            ...
            try:
                ...
                if payload.lower() in content or \
                   "sql" in content or "syntax" in content or \
                   "alert(1)" in content or "etc/passwd" in content:
...
def generate_verification_report():
    ...
    output_file = "vuln_verification_report.html"
    ...
    with open(input_file, "r", encoding="utf-8") as f:
        lines = [line.strip() for line in f if line.strip() and line.startswith("[VULNERABLE]")]

    if not lines:
        print("⚠️ No vulnerabilities found to include in the report.")
        return

    html = [
        "<html><head><meta charset='UTF-8'><title>Vulnerability Verification Report</title>",
        "<style>body{font-family:sans-serif;padding:20px;}table{border-collapse:collapse;width:100%;}",
        "th,td{border:1px solid #ccc;padding:8px;text-align:left;}th{background:#f0f0f0;}</style></head><body>",
        "<h2>Vulnerability Verification Report</h2>",
        "<table><tr><th>URL</th><th>Vulnerability Types</th><th>Action</th></tr>"
    ]

    for line in lines:
        try:
            url_part = line.split("] ", 1)[1]
            url, vulns = url_part.split(" => ")
            html.append(f"<tr><td><a href='{url}' target='_blank'>{url}</a></td><td>{vulns}</td><td>Verify manually in browser or use tools like sqlmap, Burp Suite, etc.</td></tr>")
        except Exception:
            continue
    ...
    print(f"✅ Manual verification report generated: {output_file}")
...
def main():
    while True:
        ...
        print("[6] Generate Manual Verification Report")
        print("[7] Deep SQL Injection Scan")
        print("[0] Exit")
        ...
        if choice == "1":
            ...
            dorks = generate_dorks(site, count, category)
            preview_dorks(dorks)
            save_dorks(dorks)
